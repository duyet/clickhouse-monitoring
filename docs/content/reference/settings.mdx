...
To change the active backend, set the relevant server-side env vars and redeploy. See [Conversation history backends](/guide/ai-agent) in the AI Agent docs for the full env-var reference and setup steps.
...
- [Feature Permissions](/operate/advanced/feature-permissions) — full reference for `CHM_FEATURE_*` env
- [Authentication](/operate/authentication) — setting up Clerk, Cloudflare Access, or proxy auth
- [AI Agent](/guide/ai-agent) — agent configuration and conversation store options
- [Install & Configure](/operate/deploy) — platform-specific deployment with all env vars
