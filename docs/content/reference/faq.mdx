...
1. Set `CHM_FEATURE_CONVERSATION_DB=true` at build time (before `bun run build`; the client `VITE_FEATURE_CONVERSATION_DB` is derived from it). Also requires Clerk auth.
