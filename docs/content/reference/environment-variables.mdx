...
Position N across all four variables maps to host index N. All four must have the same number of comma-separated values.
...
| Variable | Default | Description |
|---|---|---|
| `CRON_SECRET` | — | Shared secret guarding `/api/cron/health-sweep`. Send as `Authorization: Bearer <secret>` or `?secret=`. When unset, the endpoint is open. |
...
> Without `CHM_PROXY_AUTH_SECRET`, any client can forge `X-Forwarded-User` on a publicly-reachable deployment.
