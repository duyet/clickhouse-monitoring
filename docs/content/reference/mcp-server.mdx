...
- **Auth required in production.** When `NODE_ENV=production` or `CHM_API_KEY_SECRET` is set, unauthenticated requests are rejected.
- **Fail closed.** If auth is required but `CHM_API_KEY_SECRET` is unset, the endpoint returns `503` rather than serving open requests.
