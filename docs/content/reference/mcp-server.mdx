...
See [API Keys](/docs/authentication/api-keys) for setup.
...
See [Authentication](/docs/authentication) for provider setup.

---

## Available tools

The MCP server exposes the same read-only capabilities as the dashboard agent. For a full list of tools, see [AI Agent — Capabilities](/docs/ai-agent/capabilities).
...
- **Auth required in production.** When `NODE_ENV=production` or `CHM_API_KEY_SECRET` is set, unauthenticated requests are rejected. The MCP endpoint accepts either a `chm_` API key (`CHM_API_KEY_SECRET`) or a Clerk OAuth bearer token (`CHM_AUTH_PROVIDER=clerk`). See the [Authentication](#authentication) section above.
- **Fail closed.** If auth is required, `CHM_API_KEY_SECRET` is unset, and no auth provider (`CHM_AUTH_PROVIDER`) is configured, the endpoint returns `503` rather than serving open requests.
- **Open in development.** With `NODE_ENV` not set to `production` and no `CHM_API_KEY_SECRET`, the endpoint is open. Keep it on a trusted network.
...
See [Environment Variables](/docs/reference/environment-variables) for `CHM_API_KEY_SECRET` and auth variables.
See [Features — MCP](/docs/features/mcp) for more on the MCP feature and how to gate it.
