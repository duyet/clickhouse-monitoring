...
Existing domains under `apps/dashboard/src/lib/query-config/declarative/catalog/`:
...
Export a `const` typed as `DeclarativeQueryConfig`.
...
Your check is live in the catalog but the dashboard only reads from it when
`CHM_CONFIG_SOURCE=declarative` is set in the environment. The default is `ts`.
Document this in your PR if relevant.
</Step>
<Step>
### Open a PR

Open a PR against `main`. The CI `unit-tests` job runs the full bun test
