...
|---|---|---|
| `CRON_SECRET` | (unset = open) | Guards `GET /api/cron/health-sweep`. Pass as `Authorization: Bearer <secret>` or `?secret=<secret>`. Leave unset only in trusted private networks. |
