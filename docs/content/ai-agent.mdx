...
The default history backend is browser localStorage. Enable server persistence
with:
...
Supported stores are AgentState, Cloudflare D1, SQLite-backed Durable Objects,
ClickHouse, Postgres, memory, and localStorage. Server stores require
authenticated user identity; unauthenticated sessions keep local browser
history. See [Agent Conversation Storage](/docs/advanced/agent-conversation-storage)
for backend-specific setup and permissions.
