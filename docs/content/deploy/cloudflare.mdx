...
The TanStack Start app (`apps/dashboard-tsr`) uses the `@cloudflare/vite-plugin` to build a native Cloudflare Workers bundle — no OpenNext adapter required. The deploy script is just `bun run build && wrangler deploy`.
...
cd clickhouse-monitoring/apps/dashboard-tsr
bun install
