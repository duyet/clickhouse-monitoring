...
Without `CHM_PROXY_AUTH_SECRET`, the trusted-header provider is disabled. See [Authentication](/docs/authentication).
...
server {
    ...
    proxy_set_header X-Chm-Proxy-Secret "a-long-random-secret";
...
Keep `LLM_API_KEY` server-side — never set it as `VITE_*` or `NEXT_PUBLIC_*`.
