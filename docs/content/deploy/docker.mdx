...
`VITE_*` vars are inlined at build time. If you run the pre-built image, these must match what the image was built with. For a custom build, set them at build time. See [Authentication](/authentication).
</Callout>
