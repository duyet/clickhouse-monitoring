...
`VITE_*` vars are build-time inlined; if you run the pre-built image, these must match what the image was built with. For a custom build, set them at build time. See [Authentication](/docs/authentication).
...
Keep `LLM_API_KEY` server-side; never put it in a `VITE_*` or `NEXT_PUBLIC_*` variable.
...
Call the endpoint with `Authorization: Bearer <CRON_SECRET>` or `?secret=<CRON_SECRET>`.
