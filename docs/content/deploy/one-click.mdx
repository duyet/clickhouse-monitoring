...
In the Railway dashboard, choose **New Project → Deploy from image** and set the image to `ghcr.io/duyet/chmonitor:latest`.
</Step>
...
Render reads `render.yaml`, pulls `ghcr.io/duyet/chmonitor:latest`, and creates the service. Set `CLICKHOUSE_PASSWORD` as a **Secret** in the Render environment (do not store it in plain text in `render.yaml`).
</Step>
</Steps>
