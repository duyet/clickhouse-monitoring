...
- [ ] Set `CHM_API_KEY_SECRET` whenever you expose `/api/mcp`. MCP clients send long-lived tokens — require an API key.
...
- [ ] `memory` and `local` stores are ephemeral — no backup needed, but history is lost on restart.
