...
All server-side backends require `VITE_FEATURE_CONVERSATION_DB=true` set **before** `bun run build`. This is a `VITE_*` variable — it is baked into the bundle at build time and cannot be changed without a rebuild. `CONVERSATION_STORE_BACKEND` is a runtime variable and can be set or changed without rebuilding.
</Callout>
