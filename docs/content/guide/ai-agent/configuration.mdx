...
- **Cloudflare Workers** — `wrangler secret put LLM_API_KEY` for secrets; `[vars]` in `wrangler.toml` for non-secret values in your own worker. (In this repo's hosted deploy, non-secret config lives in `apps/dashboard/.env.cloud` and there is no `[vars]` block.) See [Deploy to Cloudflare](/operate/deploy/cloudflare).
