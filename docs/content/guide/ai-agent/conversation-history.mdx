...
<Mermaid chart={`flowchart TD
    A[User sends message] --> B{CHM_FEATURE_CONVERSATION_DB=true?}
...
Server stores require an authenticated user identity to namespace threads. If the request is unauthenticated, the server skips persistence and the browser keeps local history. To enable server persistence, configure an auth provider — see [Authentication](/operate/authentication).
