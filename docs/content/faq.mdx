...
Use comma-separated values for all four host variables, in the same order:
...
All four must have the same number of entries. A host selector appears in the UI when more than one host is configured.
...
Then configure a backend store (AgentState, D1, ClickHouse, Postgres). See [Agent Conversation Storage](/docs/advanced/agent-conversation-storage).
