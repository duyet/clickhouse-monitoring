...
All four must have the same number of comma-separated values. `CLICKHOUSE_NAME` is optional — if omitted, hosts are labeled by their index (Host 0, Host 1, …).
...
See [Custom Name](/docs/advanced/custom-name) for display labels, and [Environment Variables](/docs/reference/environment-variables) for the full connection reference.
