...
Gating a feature with `access = "authenticated"` requires an auth provider. Set one of:
...
> Legacy Next.js app: use `NEXT_PUBLIC_AUTH_PROVIDER` and `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` instead of the `VITE_*` names.

**Agent API token (agent APIs only):**
...
See [Authentication](/docs/authentication) for the full auth model, including API keys and reverse-proxy options.
...
- [Authentication](/docs/authentication) — auth providers and API keys.
- [Environment Variables — Feature Permissions](/docs/reference/environment-variables#feature-permissions) — full env var reference.
- [Features](/docs/features) — what each feature does.
