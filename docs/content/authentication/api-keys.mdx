...
- The `/api/mcp` endpoint also accepts Clerk OAuth bearer tokens. See [Clerk](/docs/authentication/clerk) and [MCP Server](/docs/reference/mcp-server).
- Feature-level permissions apply on top of the key auth. See [Feature Permissions](/docs/advanced/feature-permissions).
