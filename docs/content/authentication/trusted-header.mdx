...
- Cloudflare Access (`CHM_CF_ACCESS_*`) can also be configured under `CHM_AUTH_PROVIDER=proxy`. Both mechanisms are checked independently; Access is checked first. See [Cloudflare Access](/docs/authentication/cloudflare-access).
- Combine with `CHM_API_KEY_SECRET` to allow MCP/script access via `chm_` tokens. See [API keys](/docs/authentication/api-keys).
