...
| `ANYROUTER_BASE` | AnyRouter | AnyRouter base URL. |
...
- **Cloudflare Workers** — `wrangler secret put LLM_API_KEY` for secrets; `[vars]` in `wrangler.toml` for non-secret values. See [Deploy to Cloudflare](/docs/deploy/cloudflare).
- **Docker** — `environment:` block in `docker-compose.yml` or `--env-file .env`. See [Deploy with Docker](/docs/deploy/docker).
- **Kubernetes** — `Secret` for keys, `ConfigMap` for non-secret values. See [Deploy to Kubernetes](/docs/deploy/k8s).
