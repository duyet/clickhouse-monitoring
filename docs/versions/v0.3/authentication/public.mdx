...
If you also set `CHM_API_KEY_SECRET`, the API key layer activates for non-browser callers — `/api/v1/*` will require a `chm_` token from scripts and MCP clients even though the browser dashboard remains open. See [API keys](/docs/authentication/api-keys).
...
- [Clerk](/docs/authentication/clerk) — add browser sign-in with Clerk accounts
- [Cloudflare Access](/docs/authentication/cloudflare-access) — put the dashboard behind Cloudflare Access
- [Trusted header](/docs/authentication/trusted-header) — front with nginx or k8s ingress auth

To lock down programmatic access only (keep the dashboard open), set `CHM_API_KEY_SECRET` and issue `chm_` tokens to authorized callers. See [API keys](/docs/authentication/api-keys).
