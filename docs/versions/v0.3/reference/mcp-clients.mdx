...
The server-side reference (endpoint, tools, security model) is at [MCP Server](/docs/reference/mcp-server).
...
See [MCP Server](/docs/reference/mcp-server) for the full security model, resource URIs, and pre-built prompts.
