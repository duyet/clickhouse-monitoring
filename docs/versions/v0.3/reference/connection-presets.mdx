...
`readonly = 2` allows `SELECT` and `SHOW` but blocks all writes and settings changes from the session itself. `allow_ddl = 0` is redundant under `readonly = 2` but makes the intent explicit.
...
> **Note on system table access in ClickHouse Cloud:** Some `system.*` tables are restricted at certain service tiers (e.g. `system.zookeeper` is not available on Serverless). See [Platform Support Matrix](/docs/reference/support-matrix) for per-table availability across distributions and versions.
...
See [Environment Variables](/docs/reference/environment-variables) for the full variable reference, and [Multiple Hosts](/docs/advanced/multiple-hosts) for multi-host setup.
