/**
 * Shared feature-permission helpers for CHM apps.
 *
 * Normalization, resolution, and merge logic — extracted from both dashboard
 * apps so there's one place to update for v0.3+.
 */
...
export function normalizeFeatureAccess(value: string): FeatureAccess {
  ...
  throw new Error(
    `Invalid feature access "${value}". Expected one of: ${FEATURE_ACCESS_VALUES.join(', ')}, guest.`
  )
}
...
export function isFeatureAllowed(
  ...
  // Interaction-gated features render their UI for everyone and gate
  // only the auth-requiring interaction (e.g. agent sends). This takes
  // precedence over the access gate below.
