# dashboard-tsr — ClickHouse Monitoring (TanStack Start)

The ClickHouse monitoring dashboard on **TanStack Start + Cloudflare Workers** —
the migration target that replaces the Next.js app in `apps/dashboard`. Live at
...
Same features, leaner substrate. Versus `apps/dashboard` (measured, post-merge main):

| | dashboard (Next) | dashboard-tsr |
|---|---|---|
...
`.github/workflows/cloudflare.yml` (the `dashboard-tsr` job builds and deploys to
`dash-tsr.chmonitor.dev`; PRs deploy to `preview.dash-tsr.chmonitor.dev`).
...
This app carries its **own `bun.lock`** and installs in isolation — the same
pattern as `apps/landing` and `apps/docs`. Adding it to the root `workspaces`
would inject the Vite + TanStack toolchain into the lockfile shared by the Next
dashboard and the MCP worker (large, risky blast radius).
