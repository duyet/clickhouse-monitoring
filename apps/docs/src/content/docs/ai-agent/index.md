---
title: "AI Agent"
editUrl: "https://github.com/duyet/clickhouse-monitoring/edit/main/docs/content/ai-agent.mdx"
sidebar:
  label: Overview
  order: 0
---
...
A small set of tools delivers this: dedicated primitives for the common reads, and the `query` tool plus an expert **skill** recipe for everything else. See [Capabilities](/ai-agent/capabilities) for the full list.
...
**Plan and verify** — for multi-step tasks (incident triage, investigations, find-and-fix) the agent authors a live checklist with `update_plan`, keeps one step in progress, and verifies each result before stating it. Simple one-step questions skip the plan. See [Capabilities](/ai-agent/capabilities) for details.
...
For full configuration options see [Configuration](/ai-agent/configuration).

## Child pages

- [Configuration](/ai-agent/configuration) — all env vars, providers, access control
- [Capabilities](/ai-agent/capabilities) — tools, skills, plan & verify, examples
- [Conversation History](/ai-agent/conversation-history) — how history works, how to enable server persistence
  - [Store Backends](/ai-agent/conversation-history/backends) — per-backend setup (D1, ClickHouse, Postgres, …)
...
Server persistence requires `VITE_FEATURE_CONVERSATION_DB=true` and an authenticated user — configure Clerk (`VITE_AUTH_PROVIDER=clerk` + `CLERK_SECRET_KEY`) so history can be scoped per user. Unauthenticated sessions always fall back to browser history. See [Authentication](/authentication).
