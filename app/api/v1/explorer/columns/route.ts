...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  debug(`[GET /api/v1/explorer/columns]`, { hostId })
  ...
  if (!queryDef) {
    error(`[GET /api/v1/explorer/columns] Failed to build query`)
  ...
  if (result.error) {
    error(`[GET /api/v1/explorer/columns] Query error:`, result.error)
    return createApiErrorResponse(
      {
