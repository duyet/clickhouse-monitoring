 ...
 *
 * Returns list of databases in a ClickHouse instance
 */
...
const ROUTE_CONTEXT_BASE = { route: '/api/v1/explorer/databases' }

/**
 * Handle GET requests for database list
 */
export async function GET(request: NextRequest): Promise<Response> {
  ...
  debug(`[GET /api/v1/explorer/databases]`, { hostId })
  ...
  if (!queryDef) {
    error(`[GET /api/v1/explorer/databases] Failed to build query`)
    return createApiErrorResponse(
      {
        type: ApiErrorType.QueryError,
        message: 'Failed to build query for databases',
      },
      500,
      routeContext
    )
  ...
  if (result.error) {
    error(`[GET /api/v1/explorer/databases] Query error:`, result.error)
    return createApiErrorResponse(
      {
