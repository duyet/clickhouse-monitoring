 ...
 *
 * Returns CREATE TABLE statement for a table
 */
...
const ROUTE_CONTEXT_BASE = { route: '/api/v1/explorer/ddl' }

/**
 * Handle GET requests for DDL
 */
export async function GET(request: NextRequest): Promise<Response> {
  ...
  debug(`[GET /api/v1/explorer/ddl]`, { hostId })
  ...
  if (!queryDef) {
    error(`[GET /api/v1/explorer/ddl] Failed to build query`)
    return createApiErrorResponse(
      {
        type: ApiErrorType.QueryError,
        message: 'Failed to build query for DDL',
      },
      500,
      routeContext
    )
  ...
  if (result.error) {
    error(`[GET /api/v1/explorer/ddl] Query error:`, result.error)
    return createApiErrorResponse(
      {
