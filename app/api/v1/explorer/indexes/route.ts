 ...
 *
 * Returns index and key information for a table
 */
...
const ROUTE_CONTEXT_BASE = { route: '/api/v1/explorer/indexes' }

/**
 * Handle GET requests for indexes
 */
export async function GET(request: NextRequest): Promise<Response> {
  ...
  debug(`[GET /api/v1/explorer/indexes]`, { hostId })
  ...
  if (!queryDef) {
    error(`[GET /api/v1/explorer/indexes] Failed to build query`)
    return createApiErrorResponse(
      {
        type: ApiErrorType.QueryError,
        message: 'Failed to build query for indexes',
      },
      500,
      routeContext
    )
  ...
  if (result.error) {
    error(`[GET /api/v1/explorer/indexes] Query error:`, result.error)
    return createApiErrorResponse(
      {
