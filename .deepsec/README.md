...
Currently configured project: `clickhouse-monitor` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/clickhouse-monitor/SETUP.md` to fill in
   `data/clickhouse-monitor/INFO.md`.
...
`--project-id clickhouse-monitor` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/clickhouse-monitor/`.
