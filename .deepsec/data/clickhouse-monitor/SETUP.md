# Agent setup for `clickhouse-monitor`

This is a deepsec scanning workspace. Project `clickhouse-monitor` was just registered
(target: `..`). Setup is incomplete — `data/clickhouse-monitor/INFO.md`
...
2. **Fill in `data/clickhouse-monitor/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id clickhouse-monitor
pnpm deepsec process --project-id clickhouse-monitor
```
