...
export function createComparisonTools(hostId: number) {
  return {
    compare_time_periods: dynamicTool({
      ...
      inputSchema: z.object({
        ...
        period1Hours: z
          ...
          .max(720)
          .describe(
            'Hours ago for period 1 start (e.g., 48 for "2 days ago")'
          ),
        period1Duration: z
          .number()
          .int()
        ...
        period2Hours: z
          ...
          .min(0)
          .max(720)
          .describe('Hours ago for period 2 start (e.g., 0 for "now")'),
        period2Duration: z
          .number()
          .int()
          .min(1)
      ...
      execute: async (input: unknown) => {
        ...
        const period1End = params.period1Hours - params.period1Duration
        ...
        const [period1, period2] = await Promise.all([
          readOnlyQuery({
            ...
            query_params: {
              p1Start: params.period1Hours,
              p1End: Math.max(0, period1End),
            },
            hostId: resolved,
        ...
        return {
          metric: params.metric,
          period1: {
            label: `${params.period1Hours}h ago, ${params.period1Duration}h window`,
            data: period1,
          },
          period2: {
            label: `${params.period2Hours}h ago, ${params.period2Duration}h window`,
            data: period2,
          },
        }
      },
    ...
    compare_hosts: dynamicTool({
      ...
      inputSchema: z.object({
        hostId1: z.number().int().describe('First host index'),
        hostId2: z.number().int().describe('Second host index'),
      }),
      execute: async (input: unknown) => {
