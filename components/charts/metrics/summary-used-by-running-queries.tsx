...
export async function ChartSummaryUsedByRunningQueries({
  ...
  try {
    const { data: totalRows, error: totalError } = await fetchData<
      {
        metric: string
        total: number
    ...
    if (totalError) return <ErrorAlert {...totalError} />
  ...
  `
  try {
    const { data: todayQueryCountRows, error: todayError } = await fetchData<
      {
        query_count: number
      }[]
    >({ query: todayQueryCountSql, hostId })
    if (todayError) return <ErrorAlert {...todayError} />
